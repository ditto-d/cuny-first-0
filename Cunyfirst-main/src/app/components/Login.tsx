...
import { validateLogin, initializeDefaultAccounts } from "../utils/auth";
...
export function Login() {
  ...
  // Initialize default demo accounts and check for registration success
  useEffect(() => {
    console.log("🚀 Initializing CUNYfirst authentication system...");
    const accounts = initializeDefaultAccounts();
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    // Try authentication with stored accounts
    ...
    localStorage.setItem("username", result.account!.firstName);
    localStorage.setItem("userRole", result.account!.role);
    localStorage.setItem("userEmail", result.account!.email);

    toast.success(`Welcome back, ${result.account!.firstName} ${result.account!.lastName}!`);
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          ...
          <div className="mt-6 text-center text-sm text-gray-600 space-y-2">
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
              <p className="text-xs text-blue-900 font-medium mb-1">Demo Credentials (Pre-created Accounts)</p>
              <div className="space-y-1 text-xs">
                <p><span className="font-mono text-blue-700">student</span> / <span className="font-mono text-blue-700">student123</span></p>
                <p><span className="font-mono text-green-700">instructor</span> / <span className="font-mono text-green-700">instructor123</span></p>
                <p><span className="font-mono text-purple-700">registrar</span> / <span className="font-mono text-purple-700">registrar123</span></p>
              </div>
            </div>
            <p className="pt-2">
              Don't have an account?{" "}
              <Link to="/create-account" className="text-blue-600 hover:text-blue-700 font-medium">
                Create one
              </Link>
            </p>
            <p className="text-xs text-gray-500">
              Instructor accounts require approval • Admin accounts contact IT
            </p>
          </div>
        </div>
      </div>
